                            ...
                            <div class="row g-2">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select name="type"
                                        ...
                                        <option value="image" {{ $fileType == 'image' ? 'selected' : '' }}>Gambar (jpg,
                                            png, gif, webp)</option>
                                        <option value="document" {{ $fileType == 'document' ? 'selected' : '' }}>Dokumen
                                            (pdf, doc, xls)</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-2 d-flex align-items-end">
                                    <a href="{{ route('admin.file-manager.index', !empty($directory) ? ['dir' => $directory] : []) }}"
                            ...
                            @foreach ($directories as $dir)
                                <a href="{{ route('admin.file-manager.index', ['dir' => $dir['path']]) }}"
                                    ...
                                    <div class="text-end">
                                        ...
                                        <span class="badge bg-secondary">{{ number_format($dir['size'] / 1024 / 1024, 2) }}
                                            MB</span>
                                    ...
                                    @foreach ($files as $file)
                                        @php
                                            ...
                                            if ($directory == 'foto-karyawan') {
                                                ...
                                                $karyawan = \App\Models\Karyawan::where(
                                                    'foto',
                                                    'like',
                                                    '%' . $fileName,
                                                )->first();
                                            ...
                                            if ($directory == 'FilePendukung') {
                                                if (
                                                    ...
                                                    $izin = \App\Models\Izin::where(
                                                        'file_pendukung',
                                                        'like',
                                                        '%' . $fileName,
                                                    )->first();
                                                    $cuti = \App\Models\Cuti::where(
                                                        'file_pendukung',
                                                        'like',
                                                        '%' . $fileName,
                                                    )->first();
                                            ...
                                            if ($directory == 'FotoPresensi') {
                                                if (
                                                    preg_match(
                                                        '/(masuk|keluar)_(\d{4}-\d{2}-\d{2})_\d{6}/',
                                                        $fileName,
                                                        $matches,
                                                    )
                                                ) {
                                                    ...
                                                    $tanggalAbsen = \Carbon\Carbon::createFromFormat(
                                                        'd-m-Y',
                                                        $matches[1],
                                                    )->format('d M Y');
                                                    ...
                                                    if (
                                                        strpos($file['path'], '/Masuk/') !== false ||
                                                        strpos($fileName, 'Masuk') !== false
                                                    ) {
                                                        $tipeAbsen = 'Masuk';
                                                    } elseif (
                                                        strpos($file['path'], '/Keluar/') !== false ||
                                                        strpos($fileName, 'Keluar') !== false
                                                    ) {
                                                        $tipeAbsen = 'Keluar';
                                                    }
                                                }
                                        ...
                                        <tr>
                                            <td class="text-center">
                                                @if (in_array(strtolower($file['extension']), ['jpg', 'jpeg', 'png', 'gif', 'webp']))
                                                    <img src="{{ $file['url'] }}" alt="{{ $file['name'] }}"
                                                        class="img-thumbnail file-preview-img"
                                                        style="width: 50px; height: 50px; object-fit: cover; cursor: pointer;"
                                                        onclick="showImageModal('{{ $file['url'] }}', '{{ addslashes($file['name']) }}')">
                                            ...
                                            @if ($directory == 'FilePendukung')
                                                <td class="text-white-50">
                                                    @if ($tipeFile)
                                                        <span
                                                            class="badge {{ $tipeFile == 'Izin' ? 'bg-info' : 'bg-warning' }}">{{ $tipeFile }}</span>
                                                    @else
                                            ...
                                            @if ($directory == 'FotoPresensi')
                                                ...
                                                <td class="text-white-50">
                                                    @if ($tipeAbsen)
                                                        <span
                                                            class="badge {{ $tipeAbsen == 'Masuk' ? 'bg-success' : 'bg-danger' }}">{{ $tipeAbsen }}</span>
                                                    @else
                                                        <span class="text-muted">-</span>
                                                    @endif
                                                </td>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <a href="{{ $file['url'] }}" download="{{ $file['name'] }}"
                                                    class="btn btn-success btn-sm me-1 download-btn"
                                                    data-bs-toggle="tooltip" title="Download">
                                                    <i class="fas fa-download"></i>
                                                </a>
                                                @if ($directory != 'foto-karyawan')
                                                    <button class="btn btn-danger btn-sm"
                                                        onclick="deleteFile('{{ $file['path'] }}', '{{ addslashes($file['name']) }}')"
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content bg-dark text-white">
                <div class="modal-header border-secondary">
                    <h5 class="modal-title"><i class="fas fa-image me-2"></i><span id="imageTitle"></span></h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body text-center">
                    <img id="imagePreview" class="img-fluid" alt="Preview" style="max-height: 70vh;">
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Fix download button - prevent event bubbling
