...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $terlambatHariIni = Presensi::where('tanggal_presensi', $today)
            ->where('status_kehadiran', 'terlambat')
            ->count();
        $hadirHariIni = Presensi::where('tanggal_presensi', $today)
        ...
        $persentaseKehadiranHariIni = $totalKaryawan > 0 
            ? round(($presensiHariIni / $totalKaryawan) * 100, 1) 
            : 0;
        ...
        // ==================== PRESENSI BULAN INI ====================
        $presensiTotalBulanIni = Presensi::whereMonth('tanggal_presensi', $currentMonth)
            ->whereYear('tanggal_presensi', $currentYear)
            ->count();
        
        $terlambatBulanIni = Presensi::whereMonth('tanggal_presensi', $currentMonth)
            ->whereYear('tanggal_presensi', $currentYear)
            ->where('status_kehadiran', 'terlambat')
            ->count();
        
        // Rata-rata keterlambatan
        $avgKeterlambatan = Presensi::whereMonth('tanggal_presensi', $currentMonth)
            ->whereYear('tanggal_presensi', $currentYear)
            ->where('keterlambatan_menit', '>', 0)
            ->avg('keterlambatan_menit');
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $hadirCount = Presensi::where('tanggal_presensi', $date)
                ->where('status_kehadiran', 'hadir')
                ->count();
            $terlambatCount = Presensi::where('tanggal_presensi', $date)
                ->where('status_kehadiran', 'terlambat')
                ->count();
        ...
        $topTerlambat = Karyawan::select('karyawan.*', 
            DB::raw('COUNT(presensi.id_presensi) as total_terlambat'),
            DB::raw('SUM(presensi.keterlambatan_menit) as total_menit_terlambat'))
            ...
            ->where('presensi.status_kehadiran', 'terlambat')
        ...
        $karyawanPerfect = Karyawan::select('karyawan.*', 
            DB::raw('COUNT(presensi.id_presensi) as total_hadir'))
            ->join('presensi', 'karyawan.id_karyawan', '=', 'presensi.id_karyawan')
            ->whereMonth('presensi.tanggal_presensi', $currentMonth)
            ->whereYear('presensi.tanggal_presensi', $currentYear)
        ...
        $statusBulanIni = [
            'hadir' => Presensi::whereMonth('tanggal_presensi', $currentMonth)
                ->whereYear('tanggal_presensi', $currentYear)
                ->where('status_kehadiran', 'hadir')
                ->count(),
            'terlambat' => Presensi::whereMonth('tanggal_presensi', $currentMonth)
                ->whereYear('tanggal_presensi', $currentYear)
                ->where('status_kehadiran', 'terlambat')
                ->count(),
            'izin' => Presensi::whereMonth('tanggal_presensi', $currentMonth)
                ->whereYear('tanggal_presensi', $currentYear)
                ->where('status_kehadiran', 'izin')
                ->count(),
            'sakit' => Presensi::whereMonth('tanggal_presensi', $currentMonth)
                ->whereYear('tanggal_presensi', $currentYear)
                ->where('status_kehadiran', 'sakit')
                ->count(),
            'cuti' => Presensi::whereMonth('tanggal_presensi', $currentMonth)
                ->whereYear('tanggal_presensi', $currentYear)
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $izinCount = Izin::whereMonth('tanggal_pengajuan', $month->month)
                ->whereYear('tanggal_pengajuan', $month->year)
                ->count();
            $cutiCount = Cuti::whereMonth('tanggal_pengajuan', $month->month)
                ->whereYear('tanggal_pengajuan', $month->year)
                ->count();
